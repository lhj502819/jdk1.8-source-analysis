...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         */
        /*因为超时或者中断，节点会被设置为取消状态，被取消的节点是不会参与到竞争中的，他会一直保持取消状态不会转变为其他状态*/
        ...
        // 也就是说节点类型（独占和共享）和等待队列中的后继节点公用同一个字段
        Node nextWaiter;
     ...
     */
    //都是通过CAS方法来设置节点的，都是线程安全的，AQS通过死循环的方式来保证节点的正确添加，只有添加成功之后才返回，否则一直执行下去
    //通过CAS操作使添加节点的行为变成"串行化"，避免出现并发问题
    private Node enq(final Node node) {
        //无限循环尝试，直到成功为止
        for (; ; ) {
            Node t = tail;
            if (t == null) { // Must initialize
                //如果尾节点不存在，则将当前节点设置为首节点
    ...
    private Node addWaiter(Node mode) {
        ...
        //多次尝试添加尾节点
        //先通过快速尝试设计尾节点，如果失败，则调用enq(Node node)方法设置尾节点
        enq(node);
        return node;
    }
     ...
     */
    //主要是用来把当前线程挂起，从而阻塞住线程的调用栈，同时返回线程的中断状态。
    //内部调用LockSupport.park()方法来阻塞该方法。
    //当线程释放同步状态之后需要唤醒该线程所属节点的后继节点
     ...
     */
    //自旋过程，当node加入到队列中之后就需要不断的自旋尝试去获取同步状态
        ...
        try {
            ...
            for (; ; ) {
                ...
                if (p == head && tryAcquire(arg)) {
                    //获取同步状态成功就将当前节点设置为头节点
                    setHead(node);
                    //将原来的头节点的next设置为null，方便被回收
                    p.next = null; // help GC
                    failed = false;
                    return interrupted;
                }
    ...
    private boolean doAcquireNanos(int arg, long nanosTimeout)
        ...
        try {
            for (; ; ) {
                ...
                if (shouldParkAfterFailedAcquire(p, node) &&
                        //如果nanosTimeout大于spinForTimeoutThreshold，就需要等待nanosTimeout时间
                        //如果在nanosTimeout时间内没有获取到同步状态，将会从等待逻辑中自动返回
     ...
     */
    //tryAcquier：尝试获取锁，获取成功则返回true，否则返回false。该方法需要子类自己实现，必须要保证线程安全的获取同步状态
     ...
     */
    // acquire(int arg)方法为AQS提供的模板方法，该方法为独占式获取同步状态，对中断不敏感
    // 当线程因为没有获取到同步状态而进入阻塞队列如果当前线程被中断，线程不会从同步队列中移除。
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&
                //如果tryAcquire返回false，则会执行后边的addWaiter(),将当前前程加入到等待队列的尾节点
                //调用acquireQueued(Node node,int arg)方法使得该节点以“死循环的方式”获取同步状态
